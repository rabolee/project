        ...
        while True:
            ...
            # 2. 시나리오
                ...
                if self.nowMp < (self.maxMp * 0.4):  # mp -40%보다 부족하면 실행 안됨
                    ...
                    # 죽은 파티원 부활
                    for i in range(len(party_list)):
                        if party_list[i].nowHp == 0 and party_list[i].nowMp == 0:
...
def monster_list_fc():
    ...
    ilsung = Monster("일성김", 1, random.randrange(1000, 3001), random.randrange(1000, 3001))
            ...
            if party_list[i].nowHp == 0:
                ...
                if choice == '1':         # 공격
                    ...
                    if monster.nowHp <= 0:
                        ...
                        print("\n🎉전투에서 승리했습니다🎉")
                        print("💪파티원의 공격력과 HP, MP가 5%씩 상승합니다💪")
                        for j in range(len(party_list)):
                            reward(party_list[j])
                        for j in range(len(party_list)):
                            party_list[j].print_stat()  # Party 클래스의 print_stat 메서드로 파티원의 정보 불러옴
                        return party_list, monster_list
                    else:
